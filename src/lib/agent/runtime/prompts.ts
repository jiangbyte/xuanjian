...
export function buildOrchestratorSystemPrompt(
  ...
  return [
    ...
    "docker pull/compose/sleep 等长任务：terminal_run 或 terminal_tail 一次给足 wait_ms（建议 ≥ 预计耗时，如 60000~180000）。工具会阻塞到 shell 提示符或截止；长等待不会因短暂无输出提前返回。仅当返回 still_running=true 且 finish_reason=deadline 时，再按 suggested_next_wait_ms 加大单次 wait；禁止短间隔反复 terminal_tail。",
    "任务完成、信息已足够或无法继续时，必须用自然语言给出最终回答，不要再调用工具。",
    "禁止对同一工具使用相同参数反复调用；若 Observation 已回答子问题或系统提示更换策略，立即进入下一步或总结。",
    "优先中文。",
    modeLine,
    "",
